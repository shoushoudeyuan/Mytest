...
<body>
	<div class="list_title">
		<span class="layui-parent-title-customer">HOME</span>
   		...
   		<span class="layui-children-title-customer">用户转化率</span>
	</div>
	...
	<!-- 	起始月份<input id="createStartTime" type="text" name="createStartTime"
			class="easyui-datebox" />结束月份<input id="createEndTime" type="text"
			name="createEndTime" class="easyui-datebox" /> <input type="button"
			onclick="SetChart()"
			class="text_input_button text_input_button_right" value="查询"> -->
	</div>
	<!-- 为ECharts准备一个具备大小（宽高）的Dom -->
	 <div class="table_data">
		<h4 class="table_data_title">用户转化率</h4>
		...
		function SetChart(){
			...
			require([ 'echarts', 'echarts/chart/line' ], function(ec) {
		        ...
		        var option = {
                        ...
                        //图例配置
                        legend: {
                            data: ['注册数','下单人数','转化率'], //这里需要与series内的每一组数据的name值保持一致
                            y:"bottom"
                        },
                        //工具箱配置
                        toolbox: {
		        ...
		        $.ajax({
		            ...
		            url : "<%=request.getContextPath()%>/numericalController/registerPer",
		            dataType : "json",
		            async: false,//同步
		            success : function(result) {
		            	option.xAxis.data =JSON.parse(result.retDate);
		            	option.series[0].data = JSON.parse(result.rArr);
		            	option.series[1].data = JSON.parse(result.cArr);
		            	option.series[2].data = JSON.parse(result.iArr);
